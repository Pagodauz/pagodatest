...
fclose($handle);
 ...
 $menu1 = [["Ramazon taqvimi","💥 Bot azolari 👱"],["🎩 Admin bilan bog'lanish 🎩","Ortga qaytish"]];
...
Manba: [Pagoda_Uz](telegram.me/pagoda_Uz) [@Pagoda_Uz]", 'parse_mode' => 'markdown'];
 ...
 if ($text == 'Rasm') {
        ...
        if ($text == '/stat' || $text == "💥 Bot azolari 👱"){ 
        $keyfd = buildKeyBoard($menu1, $onetime = false, $resize = false);
        $content = ['chat_id' => $chat_id, 'reply_markup' => $keyfd, 'text' => "*💥Bot azolari:* \n _👱Foydalanuvchilar:_  *$private*ta  \n _👑Admin_  @xvest
...
@Pagoda_Uz ", 'parse_mode' => 'markdown'];
 ...
 if ($text == 'Soat') {
        ...
        if ($text == '/feedback' || $text == "🎩 Admin bilan bog'lanish 🎩"){
